...
test('running it twice does not double the week: identical target shifts are skipped, including duplicates in the source', () => {
  const first = planCopyWeek({ ...base, previousWeekShifts: [shift(), shift()] });
  assert.equal(first.rows.length, 1);
  assert.equal(first.skippedDuplicate, 1);
